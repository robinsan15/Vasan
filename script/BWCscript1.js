...
function addStorySelect(){
  $("#story").append("<option value='-1'>เลือกสตอรี่</option>");
  for(var i=0; i <storyName.length; i++)
  {
...
function addChapterSelect(){
  ...
  $("#chapter").append("<option value='-1'>เลือกตอน</option>");
...
function addStageSelect(){
  ...
  if(chapter < 0) return;
  $("#stage").append("<option value='-1'>เลือกStage</option>");
...
function changeStage(){
  ...
  table += '<th>Stage</th>';
  table += '<th>คะแนนต้องใช้</th>';
  table += "<th class='empathy'><img src='resource/empathy.jpg' width='15px'> การเอาใจใส่</th>";
  table += "<th class='passion'><img src='resource/passion.jpg' width='15px'> ความหลงใหล</th>";
  table += "<th class='stamina'><img src='resource/stamina.jpg' width='15px'> ความแข็งแกร่ง</th>";
  table += "<th class='wisdom'><img src='resource/wisdom.jpg' width='15px'> ความฉลาด</th>";
...
function calculate(){
  ...
  $("#result").empty();
  $("#result").append('คะแนนรวม  => ');
  $("#result").append(totalScore);
  if(stage['score'] == 0){
    $("#result").append(" <b>ขออภัย! ไม่มีข้อมูลคะแนน! จะได้รับการอัปเดตในไม่ช้า!</b>");
  }
  else if(totalScore >= stage['score']){
    $("#result").append(' <b>สำเร็จ!!!</b>');
  } else{
    $("#result").append(' <b>ไม่สำเร็จ...</b>');
  }
